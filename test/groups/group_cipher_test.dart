...
void main() {
  ...
  test('testNoSession', () async {
    ...
    var sentAliceDistributionMessage = await aliceSessionBuilder.create(GROUP_SENDER);
...
//    bobSessionBuilder.process(GROUP_SENDER, receivedAliceDistributionMessage);
    ...
    var ciphertextFromAlice = await aliceGroupCipher.encrypt(utf8.encode('smert ze smert'));
    try {
  ...
  test('testBasicEncryptDecrypt', () async {
    ...
    var sentAliceDistributionMessage = await aliceSessionBuilder.create(GROUP_SENDER);
    var receivedAliceDistributionMessage = SenderKeyDistributionMessageWrapper.fromSerialized(sentAliceDistributionMessage.serialize());
    await bobSessionBuilder.process(GROUP_SENDER, receivedAliceDistributionMessage);
  ...
  test('testBasicRatchet', () async {
    ...
    var sentAliceDistributionMessage = await aliceSessionBuilder.create(aliceName);
    var receivedAliceDistributionMessage = SenderKeyDistributionMessageWrapper.fromSerialized(sentAliceDistributionMessage.serialize());

    await bobSessionBuilder.process(aliceName, receivedAliceDistributionMessage);

    var ciphertextFromAlice = await aliceGroupCipher.encrypt(utf8.encode('smert ze smert'));
    var ciphertextFromAlice2 = await aliceGroupCipher.encrypt(utf8.encode('smert ze smert2'));
    var ciphertextFromAlice3 = await aliceGroupCipher.encrypt(utf8.encode('smert ze smert3'));
    ...
    var plaintextFromAlice2 = await bobGroupCipher.decrypt(ciphertextFromAlice2);
    var plaintextFromAlice3 = await bobGroupCipher.decrypt(ciphertextFromAlice3);
  ...
  test('testLateJoin', () async {
    ...
    for (var i = 0; i < 100; i++) {
      await aliceGroupCipher.encrypt(utf8.encode('up the punks up the punks up the punks'));
    }
    ...
    await bobSessionBuilder.process(aliceName, SenderKeyDistributionMessageWrapper.fromSerialized(distributionMessageToBob.serialize()));

    var ciphertext = await aliceGroupCipher.encrypt(utf8.encode('welcome to the group'));
  ...
  test('testOutOfOrder', () async {
    ...
    for (var i = 0; i < 100; i++) {
      ciphertexts.add(await aliceGroupCipher.encrypt(utf8.encode('up the punks')));
    }
  ...
  test('testTooFarInFuture', () async {
    ...
    var tooFarCiphertext = await aliceGroupCipher.encrypt(utf8.encode('notta gonna worka'));
    try {
