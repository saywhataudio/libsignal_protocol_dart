...
void main() {
  Future<void> runInteraction(SessionRecord aliceSessionRecord, SessionRecord bobSessionRecord) async {
    ...
    var aliceCipher = SessionCipher.fromStore(aliceStore, SignalProtocolAddress('+14159999999', 1));
    var bobCipher = SessionCipher.fromStore(bobStore, SignalProtocolAddress('+14158888888', 1));

    var alicePlaintext = utf8.encode('This is a plaintext message.');
    var message = await aliceCipher.encrypt(alicePlaintext);
    var bobPlaintext = await bobCipher.decryptFromSignal(SignalMessage.fromSerialized(message.serialize()));
    ...
    var receivedReply = await aliceCipher.decryptFromSignal(SignalMessage.fromSerialized(reply.serialize()));
    ...
    for (var i = 0; i < 50; i++) {
      ...
      aliceCiphertextMessages.add(await aliceCipher.encrypt(utf8.encode('смерть за смерть $i')));
    }
    ...
    for (var i = 0; i < aliceCiphertextMessages.length / 2; i++) {
      var receivedPlaintext = await bobCipher.decryptFromSignal(SignalMessage.fromSerialized(aliceCiphertextMessages[i].serialize()));
    ...
    for (var i = 0; i < 20; i++) {
      ...
      bobCiphertextMessages.add(await bobCipher.encrypt(utf8.encode('смерть за смерть $i')));
    }
    ...
    for (var i = 0; i < bobCiphertextMessages.length / 2; i++) {
      var receivedPlaintext = await aliceCipher.decryptFromSignal(SignalMessage.fromSerialized(bobCiphertextMessages[i].serialize()));
      assert(eq(receivedPlaintext, bobPlaintextMessages[i]));
    }

    for (var i = aliceCiphertextMessages.length ~/ 2; i < aliceCiphertextMessages.length; i++) {
      var receivedPlaintext = await bobCipher.decryptFromSignal(SignalMessage.fromSerialized(aliceCiphertextMessages[i].serialize()));
      assert(eq(receivedPlaintext, alicePlaintextMessages[i]));
    }

    for (var i = bobCiphertextMessages.length ~/ 2; i < bobCiphertextMessages.length; i++) {
      var receivedPlaintext = await aliceCipher.decryptFromSignal(SignalMessage.fromSerialized(bobCiphertextMessages[i].serialize()));
      assert(eq(receivedPlaintext, bobPlaintextMessages[i]));
    }
  }
  ...
  void initializeSessionsV3(SessionState aliceSessionState, SessionState bobSessionState) {
    ...
    var aliceIdentityKey = IdentityKeyPair(IdentityKey(aliceIdentityKeyPair.publicKey), aliceIdentityKeyPair.privateKey);
    ...
    var bobIdentityKey = IdentityKeyPair(IdentityKey(bobIdentityKeyPair.publicKey), bobIdentityKeyPair.privateKey);
    ...
    RatchetingSession.initializeSessionAlice(aliceSessionState, aliceParameters);
  ...
  test('testMessageKeyLimits', () async {
    ...
    for (var i = 0; i < 2010; i++) {
      inflight.add(await aliceCipher.encrypt(utf8.encode("you've never been so hungry, you've never been so cold")));
    }

    await bobCipher.decryptFromSignal(SignalMessage.fromSerialized(inflight[1000].serialize()));
    await bobCipher.decryptFromSignal(SignalMessage.fromSerialized(inflight[inflight.length - 1].serialize()));

    try {
      await bobCipher.decryptFromSignal(SignalMessage.fromSerialized(inflight[0].serialize()));
