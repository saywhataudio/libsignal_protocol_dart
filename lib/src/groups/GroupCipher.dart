...
class GroupCipher {
  ...
  Future<Uint8List> encrypt(Uint8List paddedPlaintext) async {
    ...
    try {
      ...
      var ciphertext = aesCbcEncrypt(senderKey.cipherKey, senderKey.iv, paddedPlaintext);
      var senderKeyMessage = SenderKeyMessage(senderKeyState.keyId, senderKey.iteration, ciphertext, senderKeyState.signingKeyPrivate);
  ...
  Future<Uint8List> decrypt(Uint8List senderKeyMessageBytes) {
  ...
  Future<Uint8List> decryptWithCallback(Uint8List senderKeyMessageBytes, DecryptionCallback callback) async {
    ...
    try {
      ...
      var senderKeyMessage = SenderKeyMessage.fromSerialized(senderKeyMessageBytes);
      ...
      var plaintext = aesCbcDecrypt(senderKey.cipherKey, senderKey.iv, senderKeyMessage.ciphertext);
